...
PhysicalAI-Robotics-Manipulation-Objects 是一个自动生成机器人动作的数据集，用于在厨房环境中执行诸如拾取和放置物体等操作。该数据集是在 IsaacSim 中生成的，利用推理算法和基于优化的运动规划来自动找到任务的解决方案 。该数据集包含一个由 Kinova Gen3 机械臂构建的双手机械手。环境是厨房场景，其中的家具和电器都是程序生成的 。此数据集仅用于研究和开发。



如果您使用该数据集，请查看并遵守发布方声明的开源协议 [HuggingFace](https://huggingface.co/datasets/nvidia/PhysicalAI-Robotics-Manipulation-Objects)
