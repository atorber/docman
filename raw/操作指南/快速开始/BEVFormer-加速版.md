...
| **CPU** | 按需 | 建议按表单默认值及以上 |
| **内存** | 按需 | 建议按表单默认值及以上 |
| **GPU** | 24G以上显存 1-8 卡；P800 8卡 | 24G以上显存 1-8 卡；P800 8卡 |
| **CDS** | 按需 | 按需 |
...
![image \(4\).png](https://bce.bdstatic.com/doc/bce-doc/AIHC/image%20%284%29_d77c535.png)

* 代码保存路径：/root/workspace/BEVFormer
* 数据默认挂载路径：/mnt/dataset/autodrive
* 虚拟环境名：
    * 昆仑芯P800：python38_torch201_cuda         
    * 英伟达：uniad_new
...
# 设置训练结果保存路径，默认/root/workspace/BEVFormer/work_dir
export WORK_DIR=/root/workspace/BEVFormer/work_dir

cd /root/workspace/BEVFormer
source /root/workspace/BEVFormer/env_bevformer.sh

export PYTHONPATH=/root/workspace/BEVFormer:$PYTHONPATH 
cd /root/workspace/BEVFormer && \
...
export CONFIG=projects/configs/bevformer/bevformer_base.py
# 设置训练使用gpu卡数，默认使用全部gpu
export GPUS=$(xpu-smi 2>/dev/null | grep -c "P800")
# 设置训练结果保存路径，默认/root/workspace/BEVFormer/work_dir
export WORK_DIR=/root/workspace/BEVFormer/work_dir
...
![image \(5\).png](https://bce.bdstatic.com/doc/bce-doc/AIHC/image%20%285%29_e04f65d.png)
