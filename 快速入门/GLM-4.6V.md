...
GLM-4.6V 系列是 GLM 系列在多模态方向上的一次重要迭代。GLM-4.6V 参数规模达到 106B，是一款专为云端和高性能集群场景设计的基础模型。该系列模型首次集成了原生函数调用（Function Calling）能力，有效弥合了“视觉感知”与“可执行动作”之间的鸿沟，为现实业务场景中的多模态智能体提供了统一的技术基础。
...
可处理最多 128K tokens 的多文档或长文档输入，直接将格式丰富的页面作为图像进行解析。它能联合理解文本、版式、图表、表格和插图，无需事先转换为纯文本，即可准确理解复杂且富含图像的文档。
