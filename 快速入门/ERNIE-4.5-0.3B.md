...
## 关键技术点
### 多模态异构 MoE 预训练 
模型在文本和视觉两种模态上进行联合训练，以更好地捕捉多模态信息的细微差别，并提升在文本生成、图像理解和跨模态推理等任务上的性能。为了在实现这一目标的同时，避免单一模态对其他模态学习过程的干扰设计了异构 MoE 结构，引入了三维旋转位置编码，并采用了路由正交损失和多模态令牌平衡损失。这些架构上的选择确保了两种模态都能得到有效的表征，在训练过程中实现相互促进。
