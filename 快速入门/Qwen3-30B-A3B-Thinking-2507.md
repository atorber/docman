...
Qwen3-30B-A3B-Thinking-2507 是通义千问系列于 2025年7月 发布的最新一代大语言模型。该模型为混合专家（MoE）架构，总参数量305亿，激活参数量33亿，配置128个专家节点（每次激活8个）。它是 Qwen3-30B-A3B 系列的优化版本，专注于思考模式（即深度推理模式），通过在生成答案前进行显式的多步逻辑推导，以极高的推理精度解决复杂难题，同时凭借 MoE 架构将推理成本控制在合理范围。
