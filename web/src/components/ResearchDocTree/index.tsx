...
import { getResearchDocTree } from '../../services/api';
import { DocNode } from '../../types';

interface ResearchDocTreeProps {
  onSelectDoc: (node: DocNode) => void;
  selectedDocPath?: string;
  defaultSelectFirst?: boolean;
}

const ResearchDocTree: React.FC<ResearchDocTreeProps> = ({
  onSelectDoc,
  selectedDocPath,
  ...
  const loadTree = async () => {
    try {
      setLoading(true);
      const data = await getResearchDocTree();
      ...
      const transformToAntdTree = (nodes: DocNode[]): any[] => {
        ...
        return nodes.map((node) => {
          ...
          return {
            ...
            title: (
              <div
                style={{ display: 'flex', alignItems: 'center', padding: '4px 8px', borderRadius: 4, transition: 'background 0.2s' }}
      ...
      console.error('Failed to load research doc tree:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (treeData.length === 0) {
    return <Empty description="暂无研报文件" />;
  }
...
export default ResearchDocTree;
